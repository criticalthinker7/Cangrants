...
const GRANTS = [
  ...
  { id:9, name:"Convergent Stream \u2013 Development", org:"Canada Media Fund (CMF)", open:"2026-01-06", close:"2027-04-08", url:"https://cmf-fmc.ca/our-programs/", discipline:["Television","Digital Media"], location:"Canada", amount:"Up to $350,000", tags:["Broadcasters","Producers","Television","Digital"], eligibility:"Canadian independent producers with a broadcaster license agreement. Projects must be linear television content with an interactive digital component.", description:"Funds development of Canadian television programs with convergent digital media components." },
  { id:10, name:"Experimental Stream", org:"Canada Media Fund (CMF)", open:"2026-02-01", close:"2027-05-01", url:"https://cmf-fmc.ca/our-programs/", discipline:["Digital Media","Interactive"], location:"Canada", amount:"Up to $750,000", tags:["Innovation","Interactive","Digital","Diverse Producers"], eligibility:"Canadian companies creating innovative digital media content and applications. Projects must demonstrate market reach and commercial viability.", description:"Funds innovative Canadian digital media and interactive projects with strong market potential." },
  ...
  { id:21, name:"Sundance Documentary Fund", org:"Sundance Institute", open:"2026-03-01", close:"2027-06-01", url:"https://www.sundance.org/apply", discipline:["Documentary"], location:"International", amount:"$10,000\u2013$50,000", tags:["Documentary","Emerging","Diverse","International","Social Issues"], eligibility:"Documentary filmmakers worldwide with projects in development or production. Priority for underrepresented voices and urgent social themes.", description:"Funds documentary films from global filmmakers exploring urgent human stories and social issues." },
...
function LandingPage({ onAuth }: { onAuth: (user: UserInfo) => void }) {
  ...
  const [users, setUsers] = useState(() => {
    const demo = { email:"demo@betterhalflabs.com", password:"demo123", name:"Demo Artist", province:"Ontario", discipline:"Film" };
    try {
  ...
  const inp = (field: string, label: string, type="text", opts: string[] | null = null) => {
    ...
    return (
      <div style={{ marginBottom:14 }}>
        <label style={{ display:"block", fontSize:12, fontWeight:600, color:"#3F5A48", letterSpacing:"1px", textTransform:"uppercase", marginBottom:6 }}>{label}</label>
        {isSelect
          ? <select value={formVal} onChange={e => setForm(p=>({...p,[field]:e.target.value}))}
              style={{ width:"100%", padding:"12px 14px", background:"#FFFFFF", border:`1px solid ${errors[field]?"#E74C3C":"#C8DDD0"}`, borderRadius:8, color: formVal?"#0F3D2A":"#9AB0A2", fontSize:14, fontFamily:"'DM Sans',sans-serif", outline:"none", boxSizing:"border-box" }}>
              <option value="" disabled>Select...</option>
              {opts!.map(o => <option key={o} value={o} style={{background:"#FFFFFF",color:"#0F3D2A"}}>{o}</option>)}
            </select>
          : <input type={type} value={formVal} onChange={e => setForm(p=>({...p,[field]:e.target.value}))} placeholder={`Enter ${label.toLowerCase()}`}
              style={{ width:"100%", padding:"12px 14px", background:"#FFFFFF", border:`1px solid ${errors[field]?"#E74C3C":"#C8DDD0"}`, borderRadius:8, color:"#0F3D2A", fontSize:14, fontFamily:"'DM Sans',sans-serif", outline:"none", boxSizing:"border-box" }} />
        }
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#F4FBF7", color:"#0F3D2A", fontFamily:"'DM Sans',sans-serif", position:"relative", overflowX:"hidden" }}>
      ...
      <div style={{ position:"fixed", inset:0, pointerEvents:"none", zIndex:0 }}>
        {particles.map(p => (
          <div key={p.id} style={{ position:"absolute", left:`${p.x}%`, top:`${p.y}%`, width:p.size, height:p.size, borderRadius:"50%", background:"#2F7A52", opacity:0.18, animation:`drift ${p.dur}s ease-in-out ${p.delay}s infinite alternate` }}/>
        ))}
        <div style={{ position:"absolute", top:"-20%", left:"-10%", width:"60vw", height:"60vw", borderRadius:"50%", background:"radial-gradient(circle, rgba(168,220,192,0.45) 0%, transparent 70%)", pointerEvents:"none" }}/>
        <div style={{ position:"absolute", bottom:"-20%", right:"-10%", width:"50vw", height:"50vw", borderRadius:"50%", background:"radial-gradient(circle, rgba(47,122,82,0.10) 0%, transparent 70%)", pointerEvents:"none" }}/>
      </div>

      <div style={{ position:"relative", zIndex:10, padding:"20px 40px", display:"flex", justifyContent:"space-between", alignItems:"center", borderBottom:"1px solid #D1E5D8", background:"rgba(255,255,255,0.7)", backdropFilter:"blur(8px)" }}>
        <CanGrantsLogoImg size="sm"/>
        <div style={{ display:"flex", gap:10 }}>
          {mode !== "login" && <button onClick={() => {setMode("login"); setErrors({});}} style={{ padding:"10px 22px", borderRadius:8, border:"1px solid #2F7A52", background:"transparent", color:"#2F7A52", fontSize:14, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", fontWeight:600 }}>Sign In</button>}
          {mode !== "register" && <button onClick={() => {setMode("register"); setErrors({});}} style={{ padding:"10px 22px", borderRadius:8, border:"none", background:"#2F7A52", color:"#FFFFFF", fontSize:14, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", fontWeight:700 }}>Create Account</button>}
        </div>
      </div>
      ...
      <div style={{ position:"relative", zIndex:5, maxWidth:1100, margin:"0 auto", padding:"0 24px" }}>
        ...
        {mode === "welcome" && (
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", textAlign:"center", paddingTop:"7vh" }}>
            <div style={{ width:60, height:2, background:"#2F7A52", marginBottom:28 }}/>
            <div style={{ fontSize:12, letterSpacing:"4px", color:"#2F7A52", textTransform:"uppercase", marginBottom:20, fontWeight:600 }}>Welcome to</div>
            <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(56px,8vw,100px)", fontWeight:700, lineHeight:0.9, margin:"0 0 6px", letterSpacing:"-2px", color:"#0F3D2A" }}>
              Can<span style={{ color:"#2F7A52" }}>Grants</span>
            </h1>
            <div style={{ width:120, height:1, background:"rgba(47,122,82,0.4)", margin:"24px auto" }}/>
            <p style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(18px,2.5vw,26px)", fontStyle:"italic", color:"#3F5A48", lineHeight:1.5, maxWidth:680, marginBottom:10 }}>
              An interactive AI-powered grant tracking platform
            </p>
            <p style={{ fontSize:13, color:"#4F8C6A", letterSpacing:"2px", textTransform:"uppercase", marginBottom:6, fontWeight:600 }}>created by</p>
            <div style={{ marginBottom:32 }}><CanGrantsLogoImg size="sm"/></div>

            <div style={{ maxWidth:580, marginBottom:40 }}>
              <p style={{ fontSize:16, lineHeight:1.8, color:"#3F5A48", margin:"0 0 16px" }}>
                A single search platform for <strong style={{ color:"#0F3D2A" }}>individual artists and producers based in Canada</strong> — discover, track, and apply for arts funding from coast to coast and around the world.
              </p>
              <div style={{ display:"flex", justifyContent:"center", gap:10, flexWrap:"wrap", margin:"24px 0" }}>
                {["Funding","Labs","Residencies","Tax Credits","International Programs"].map(t => (
                  <span key={t} style={{ padding:"6px 16px", borderRadius:20, border:"1px solid #2F7A52", color:"#2F7A52", fontSize:12, fontWeight:600, letterSpacing:"0.5px", background:"#FFFFFF" }}>{t}</span>
                ))}
              </div>
            </div>

            <div style={{ background:"#FFFFFF", border:"1px solid #D1E5D8", borderRadius:20, padding:"40px 50px", maxWidth:580, width:"100%", marginBottom:50, boxShadow:"0 8px 30px rgba(47,122,82,0.08)" }}>
              <div style={{ fontSize:11, letterSpacing:"4px", color:"#2F7A52", textTransform:"uppercase", marginBottom:12, fontWeight:700 }}>CREATE \u00b7 APPLY \u00b7 TRACK</div>
              <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:32, fontWeight:700, margin:"0 0 12px", lineHeight:1.2, color:"#0F3D2A" }}>
                Explore the world of<br/>arts grants
              </h2>
              <p style={{ fontSize:14, color:"#5D7A6B", lineHeight:1.7, margin:"0 0 28px" }}>
                30+ funding opportunities \u00b7 AI-powered eligibility matching \u00b7 Proposal drafting assistant \u00b7 Deadline tracker
              </p>
              <button onClick={() => setMode("register")} style={{ width:"100%", padding:"16px", borderRadius:12, border:"none", background:"#2F7A52", color:"#FFFFFF", fontSize:16, fontWeight:700, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", letterSpacing:"0.5px", marginBottom:14 }}>
                Create Your Account {"\u2192"}
              </button>
              <button onClick={() => setMode("login")} style={{ width:"100%", padding:"13px", borderRadius:12, border:"1px solid #2F7A52", background:"transparent", color:"#2F7A52", fontSize:14, fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>
                I already have an account
              </button>
              <p style={{ fontSize:11, color:"#7C9486", marginTop:14, lineHeight:1.5 }}>Registration restricted to Canadian residents \u00b7 Postal code verified</p>
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:20, width:"100%", marginBottom:60 }}>
              {[
                ...
                <div key={f.title} style={{ background:"#FFFFFF", border:"1px solid #D1E5D8", borderRadius:14, padding:"22px 20px", textAlign:"left", boxShadow:"0 2px 12px rgba(47,122,82,0.05)" }}>
                  <div style={{ fontSize:26, marginBottom:10 }}>{f.icon}</div>
                  <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:18, fontWeight:700, marginBottom:6, color:"#0F3D2A" }}>{f.title}</div>
                  <div style={{ fontSize:13, color:"#5D7A6B", lineHeight:1.6 }}>{f.desc}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {mode === "login" && (
          <div style={{ display:"flex", justifyContent:"center", paddingTop:"6vh" }}>
            <div style={{ width:"100%", maxWidth:440, background:"#FFFFFF", padding:"36px 32px", borderRadius:16, border:"1px solid #D1E5D8", boxShadow:"0 8px 30px rgba(47,122,82,0.08)" }}>
              <button onClick={() => setMode("welcome")} style={{ background:"none", border:"none", color:"#2F7A52", cursor:"pointer", fontSize:13, marginBottom:20, padding:0, fontFamily:"'DM Sans',sans-serif", fontWeight:600 }}>{"\u2190"} Back</button>
              <div style={{ width:40, height:2, background:"#2F7A52", marginBottom:18 }}/>
              <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:36, fontWeight:700, margin:"0 0 6px", color:"#0F3D2A" }}>Sign In</h2>
              <p style={{ fontSize:14, color:"#5D7A6B", marginBottom:26 }}>Welcome back. Access your CanGrants dashboard.</p>
              <div style={{ marginBottom:14 }}>
                <label style={{ display:"block", fontSize:12, fontWeight:600, color:"#3F5A48", letterSpacing:"1px", textTransform:"uppercase", marginBottom:6 }}>Email Address</label>
                <input type="email" value={loginForm.email} onChange={e => setLoginForm(p=>({...p,email:e.target.value}))} placeholder="your@email.com"
                  style={{ width:"100%", padding:"12px 14px", background:"#FFFFFF", border:"1px solid #C8DDD0", borderRadius:8, color:"#0F3D2A", fontSize:14, fontFamily:"'DM Sans',sans-serif", outline:"none", boxSizing:"border-box" }}/>
              </div>
              <div style={{ marginBottom:22 }}>
                <label style={{ display:"block", fontSize:12, fontWeight:600, color:"#3F5A48", letterSpacing:"1px", textTransform:"uppercase", marginBottom:6 }}>Password</label>
                <input type="password" value={loginForm.password} onChange={e => setLoginForm(p=>({...p,password:e.target.value}))} placeholder="\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022"
                  onKeyDown={e => e.key==="Enter" && handleLogin()}
                  style={{ width:"100%", padding:"12px 14px", background:"#FFFFFF", border:"1px solid #C8DDD0", borderRadius:8, color:"#0F3D2A", fontSize:14, fontFamily:"'DM Sans',sans-serif", outline:"none", boxSizing:"border-box" }}/>
              </div>
              {loginErr && <div style={{ background:"rgba(192,57,43,0.10)", border:"1px solid rgba(192,57,43,0.4)", borderRadius:8, padding:"10px 14px", fontSize:13, color:"#C0392B", marginBottom:16 }}>{loginErr}</div>}
              <button onClick={handleLogin} style={{ width:"100%", padding:"14px", borderRadius:10, border:"none", background:"#2F7A52", color:"#FFFFFF", fontSize:15, fontWeight:700, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", marginBottom:14 }}>
                Sign In {"\u2192"}
              </button>
              <p style={{ textAlign:"center", fontSize:13, color:"#5D7A6B" }}>
                Don't have an account?{" "}
                <button onClick={() => {setMode("register"); setErrors({});}} style={{ background:"none", border:"none", color:"#2F7A52", cursor:"pointer", fontSize:13, fontFamily:"'DM Sans',sans-serif", fontWeight:700 }}>Create one</button>
              </p>
              <div style={{ marginTop:20, padding:"12px", background:"#EAF5EE", borderRadius:8, fontSize:12, color:"#3F5A48", textAlign:"center" }}>
                Demo: demo@betterhalflabs.com \u00b7 demo123
              </div>
            </div>
          </div>
        )}
        ...
        {mode === "register" && (
          <div style={{ display:"flex", justifyContent:"center", paddingTop:"4vh", paddingBottom:60 }}>
            <div style={{ width:"100%", maxWidth:560, background:"#FFFFFF", padding:"36px 32px", borderRadius:16, border:"1px solid #D1E5D8", boxShadow:"0 8px 30px rgba(47,122,82,0.08)" }}>
              <button onClick={() => setMode("welcome")} style={{ background:"none", border:"none", color:"#2F7A52", cursor:"pointer", fontSize:13, marginBottom:20, padding:0, fontFamily:"'DM Sans',sans-serif", fontWeight:600 }}>{"\u2190"} Back</button>
              <div style={{ width:40, height:2, background:"#2F7A52", marginBottom:18 }}/>
              <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:36, fontWeight:700, margin:"0 0 6px", color:"#0F3D2A" }}>Create Your Account</h2>
              <p style={{ fontSize:14, color:"#5D7A6B", marginBottom:8 }}>Join CanGrants — Canada's AI-powered grant platform for artists and producers.</p>
              <div style={{ background:"#EAF5EE", border:"1px solid #C8DDD0", borderRadius:8, padding:"10px 14px", fontSize:12, color:"#1E4A3A", marginBottom:26 }}>
                <strong>Canadian residents only.</strong> A valid Canadian postal code is required to create an account.
              </div>

              <div style={{ fontSize:11, fontWeight:700, color:"#2F7A52", letterSpacing:"2px", textTransform:"uppercase", marginBottom:14, paddingBottom:8, borderBottom:"1px solid #D1E5D8" }}>Personal Info</div>
              {inp("name","Full Name")}
              {inp("email","Email Address","email")}
              {inp("password","Password","password")}

              <div style={{ fontSize:11, fontWeight:700, color:"#2F7A52", letterSpacing:"2px", textTransform:"uppercase", margin:"20px 0 14px", paddingBottom:8, borderBottom:"1px solid #D1E5D8" }}>Canadian Address</div>
              {inp("address","Street Address")}
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                <div>{inp("city","City")}</div>
                <div>{inp("postal","Postal Code")}<div style={{ fontSize:11, color:"#7C9486", marginTop:2 }}>Format: A1A 1A1</div></div>
              </div>
              {inp("province","Province / Territory","text", CA_PROVINCES)}

              <div style={{ fontSize:11, fontWeight:700, color:"#2F7A52", letterSpacing:"2px", textTransform:"uppercase", margin:"20px 0 14px", paddingBottom:8, borderBottom:"1px solid #D1E5D8" }}>Your Practice</div>
              ...
              <button onClick={handleRegister} style={{ width:"100%", padding:"15px", borderRadius:10, border:"none", background:"#2F7A52", color:"#FFFFFF", fontSize:16, fontWeight:700, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", marginTop:10, marginBottom:12, letterSpacing:"0.5px" }}>
                Create Account & Explore Grants {"\u2192"}
              </button>
              <p style={{ textAlign:"center", fontSize:13, color:"#5D7A6B" }}>
                Already have an account?{" "}
                <button onClick={() => setMode("login")} style={{ background:"none", border:"none", color:"#2F7A52", cursor:"pointer", fontSize:13, fontFamily:"'DM Sans',sans-serif", fontWeight:700 }}>Sign in</button>
              </p>
            </div>
          </div>
        )}
      ...
      <div style={{ position:"relative", zIndex:5, borderTop:"1px solid #D1E5D8", padding:"24px 40px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:16, marginTop:20, background:"rgba(255,255,255,0.6)" }}>
        <CanGrantsLogoImg size="sm"/>
        <div style={{ fontSize:12, color:"#5D7A6B", textAlign:"right" }}>
          <div style={{ color:"#2F7A52", fontWeight:700, fontSize:13 }}>CanGrants</div>
          <div>{"\u00A9"} 2026 BetterHalf Labs {"\u00b7"} Toronto, Canada {"\u00b7"} betterhalflabs.com</div>
      ...
      <style>{`
        @keyframes drift {
          from { transform: translateY(0px) translateX(0px); opacity:0.10; }
          to { transform: translateY(-20px) translateX(10px); opacity:0.28; }
        }
      `}</style>
    </div>
  );
...
function Dashboard({ user, onLogout }: { user: UserInfo; onLogout: () => void }) {
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif", background:"#F4FBF7", minHeight:"100vh", color:"#1A2D24" }}>
      ...
      <header style={{ background:"#FFFFFF", color:"#0F3D2A", padding:"0 28px", display:"flex", alignItems:"center", justifyContent:"space-between", height:72, position:"sticky", top:0, zIndex:100, boxShadow:"0 2px 12px rgba(47,122,82,0.10)", borderBottom:"1px solid #D1E5D8" }}>
        <div style={{ display:"flex", alignItems:"center", gap:18 }}>
          <CanGrantsLogoImg size="sm" />
          <div style={{ width:1, height:32, background:"#D1E5D8" }}/>
          <span style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:26, fontWeight:700, letterSpacing:"-0.5px", color:"#0F3D2A" }}>Can<span style={{ color:"#2F7A52" }}>Grants</span></span>
        </div>
        <div style={{ display:"flex", gap:6, alignItems:"center" }}>
          <nav style={{ display:"flex", gap:4 }}>
            {[{id:"discover",label:"Discover"},{id:"saved",label:`Saved (${saved.size})`},{id:"applications",label:"My Applications"},{id:"assistant",label:"AI Assistant"},{id:"contact",label:"Contact"}].map(tab => (
              <button key={tab.id} onClick={()=>setActiveTab(tab.id)} style={{ background:activeTab===tab.id?"#2F7A52":"transparent", color:activeTab===tab.id?"#FFFFFF":"#3F5A48", border:"none", borderRadius:8, padding:"9px 18px", fontSize:14, fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", transition:"background 0.15s ease" }}>{tab.label}</button>
            ))}
          </nav>
          <div style={{ width:1, height:28, background:"#D1E5D8", margin:"0 10px" }}/>
          <div style={{ fontSize:13, color:"#5D7A6B", marginRight:10, fontWeight:500 }}>Hi, {user.name.split(" ")[0]}</div>
          <button onClick={onLogout} style={{ padding:"8px 16px", borderRadius:8, border:"1px solid #2F7A52", background:"transparent", color:"#2F7A52", fontSize:13, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", fontWeight:600 }}>Sign Out</button>
        </div>
      </header>
      ...
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"28px 20px" }}>
        ...
        {activeTab==="discover" && (
          <div>
            <div style={{ marginBottom:28, display:"flex", gap:16, flexWrap:"wrap", alignItems:"center", justifyContent:"space-between" }}>
              <div>
                <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:32, fontWeight:700, margin:"0 0 4px", color:"#0F3D2A" }}>Grant Discovery</h1>
                <p style={{ margin:0, color:"#5D7A6B", fontSize:14 }}>{GRANTS.length} opportunities \u00b7 Canadian &amp; International \u00b7 Updated 2026</p>
              </div>
              <div style={{ display:"flex", gap:12 }}>
                {[{label:"Total",value:GRANTS.length,color:"#2F7A52"},{label:"Canadian",value:GRANTS.filter(g=>g.location==="Canada").length,color:"#2D7D46"},{label:"International",value:GRANTS.filter(g=>g.location==="International").length,color:"#1A5FA8"}].map(s=>(
            ...
            <div style={{ background:"#fff", borderRadius:14, padding:20, marginBottom:24, boxShadow:"0 2px 12px rgba(0,0,0,0.06)", border:"1px solid #D1E5D8" }}>
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search grants, organizations, disciplines, tags..." style={{ width:"100%", padding:"12px 16px", borderRadius:8, border:"1.5px solid #C8DDD0", fontSize:14, fontFamily:"'DM Sans',sans-serif", background:"#FAFEFB", outline:"none", boxSizing:"border-box", marginBottom:14, color:"#1A2D24" }}/>
              <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                {[{key:"discipline",label:"Discipline",opts:ALL_DISCIPLINES},{key:"location",label:"Location",opts:["Canada","International"]},{key:"deadline",label:"Deadline",opts:[["urgent","Urgent (\u226414 days)"],["month","This Month"],["rolling","Rolling"]]},{key:"tag",label:"For\u2026",opts:ALL_TAGS}].map(({key,label,opts})=>(
                  <select key={key} value={filters[key as keyof typeof filters]} onChange={e=>setFilters(p=>({...p,[key]:e.target.value}))} style={{ padding:"8px 12px", borderRadius:8, border:"1.5px solid #C8DDD0", fontSize:13, background:"#fff", fontFamily:"'DM Sans',sans-serif", color:"#1A2D24", cursor:"pointer" }}>
                    <option value="">{label}: All</option>
                    {opts.map(o=>Array.isArray(o)?<option key={o[0]} value={o[0]}>{o[1]}</option>:<option key={o} value={o}>{o}</option>)}
                  </select>
                ))}
                {(search||Object.values(filters).some(Boolean))&&<button onClick={()=>{setSearch("");setFilters({discipline:"",location:"",tag:"",deadline:""}); }} style={{ padding:"8px 14px", borderRadius:8, border:"1px solid #C8DDD0", background:"transparent", fontSize:13, cursor:"pointer", color:"#888", fontFamily:"'DM Sans',sans-serif" }}>Clear all</button>}
              </div>
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))", gap:18 }}>
              {filtered.map(g=>{
                ...
                return (
                  <div key={g.id} style={{ background:"#fff", borderRadius:14, border:"1px solid #D1E5D8", boxShadow:"0 2px 10px rgba(0,0,0,0.05)", overflow:"hidden", display:"flex", flexDirection:"column" }}>
                    <div style={{ background:g.location==="Canada"?"#0F3D2A":"#1F5277", padding:"14px 18px", display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:10, letterSpacing:"1.5px", color:g.location==="Canada"?"#A8DCC0":"#9CC3DF", textTransform:"uppercase", marginBottom:4 }}>{g.location} \u00b7 {g.discipline.slice(0,2).join(", ")}</div>
                        <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:18, fontWeight:700, color:"#FFFFFF", lineHeight:1.2 }}>{g.name}</div>
                        <div style={{ fontSize:12, color:"#D1E5D8", marginTop:3 }}>{g.org}</div>
                      </div>
                      <button onClick={()=>toggleSave(g.id)} style={{ background:"none", border:"none", cursor:"pointer", fontSize:18, padding:"0 0 0 8px", color:isSaved?"#FFD27A":"#A8DCC0" }}>{isSaved?"\u2605":"\u2606"}</button>
                    </div>
                    <div style={{ padding:"14px 18px", flex:1, display:"flex", flexDirection:"column", gap:10 }}>
                      <p style={{ margin:0, fontSize:13, color:"#5D7A6B", lineHeight:1.5 }}>{g.description}</p>
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                        {g.tags.slice(0,3).map(t=><span key={t} style={{ background:"#E3F2EA", color:"#2A5C2A", fontSize:11, padding:"3px 8px", borderRadius:20, fontWeight:500 }}>{t}</span>)}
                      </div>
                    ...
                    <div style={{ padding:"12px 18px", borderTop:"1px solid #E0EEE5", display:"flex", gap:8 }}>
                      <button onClick={()=>setSelectedGrant(g)} style={{ flex:1, padding:"8px 0", borderRadius:8, border:"1.5px solid #C8DDD0", background:"transparent", fontSize:13, cursor:"pointer", color:"#3F5A48", fontFamily:"'DM Sans',sans-serif", fontWeight:500 }}>Details</button>
                      <button onClick={()=>{addApplication(g);setActiveTab("applications");}} style={{ flex:1, padding:"8px 0", borderRadius:8, border:"none", background:hasApp?"#E8F5E8":"#0F3D2A", color:hasApp?"#1A7A3A":"#FFFFFF", fontSize:13, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", fontWeight:600 }}>{hasApp?"\u2713 Tracking":"Track"}</button>
                      <a href={g.url} target="_blank" rel="noopener noreferrer" style={{ flex:1, padding:"8px 0", borderRadius:8, border:"none", background:"#2F7A52", color:"#0F3D2A", fontSize:13, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", fontWeight:600, textDecoration:"none", textAlign:"center", lineHeight:"1.8" }}>Apply {"\u2197"}</a>
                    </div>
                  </div>
                );
              })}
        ...
        {activeTab==="saved" && (
          <div>
            <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:32, fontWeight:700, marginBottom:8, color:"#0F3D2A" }}>Saved Grants</h1>
            <p style={{ color:"#5D7A6B", fontSize:14, marginBottom:24 }}>{saved.size} grants saved to your list</p>
            {savedGrants.length===0?(<div style={{ textAlign:"center", padding:"60px 0", color:"#888" }}><div style={{ fontSize:40, marginBottom:12 }}>\u2606</div><p>No saved grants yet. Star grants in Discover.</p></div>):(
              <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                {savedGrants.map(g=>{
                  ...
                  return (
                    <div key={g.id} style={{ background:"#fff", borderRadius:12, border:"1px solid #D1E5D8", padding:"18px 22px", display:"flex", gap:18, alignItems:"center", boxShadow:"0 1px 8px rgba(0,0,0,0.04)" }}>
                      <div style={{ width:6, alignSelf:"stretch", background:g.location==="Canada"?"#2D7D46":"#1A5FA8", borderRadius:3, flexShrink:0 }}/>
                      <div style={{ flex:1 }}>
                        <div style={{ display:"flex", gap:8, alignItems:"baseline", marginBottom:3 }}>
                          <span style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:18, fontWeight:700, color:"#0F3D2A" }}>{g.name}</span>
                          <span style={{ fontSize:12, color:"#888" }}>\u00b7 {g.org}</span>
                        </div>
                        <p style={{ margin:"4px 0", fontSize:13, color:"#5D7A6B" }}>{g.eligibility.slice(0,120)}\u2026</p>
                        <div style={{ display:"flex", gap:8, marginTop:6, flexWrap:"wrap" }}>
                          {g.tags.slice(0,3).map(t=><span key={t} style={{ background:"#E3F2EA", color:"#2A5C2A", fontSize:11, padding:"2px 7px", borderRadius:12 }}>{t}</span>)}
                        </div>
                      </div>
                      <div style={{ display:"flex", flexDirection:"column", gap:8, alignItems:"flex-end", flexShrink:0 }}>
                        ...
                        <div style={{ display:"flex", gap:6 }}>
                          <button onClick={()=>toggleSave(g.id)} style={{ padding:"6px 12px", borderRadius:7, border:"1px solid #C8DDD0", background:"transparent", fontSize:12, cursor:"pointer", color:"#888" }}>Remove</button>
                          <button onClick={()=>{addApplication(g);setActiveTab("applications");}} style={{ padding:"6px 12px", borderRadius:7, border:"none", background:hasApp?"#E8F5E8":"#0F3D2A", color:hasApp?"#1A7A3A":"#FFFFFF", fontSize:12, cursor:"pointer", fontWeight:600 }}>{hasApp?"\u2713 Tracked":"Track"}</button>
                        </div>
                      </div>
                    </div>
                  );
        ...
        {activeTab==="applications" && (
          <div>
            <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:32, fontWeight:700, marginBottom:8, color:"#0F3D2A" }}>My Applications</h1>
            <p style={{ color:"#5D7A6B", fontSize:14, marginBottom:24 }}>Track your grant applications and their status</p>
            {["Not Started","In Progress","Submitted"].map(status=>{
              ...
              return (
                <div key={status} style={{ marginBottom:28 }}>
                  ...
                  {apps.length===0?(<div style={{ background:"#FAFEFB", borderRadius:10, padding:"20px", textAlign:"center", color:"#bbb", fontSize:13, border:"1px dashed #C8DDD0" }}>No grants here yet</div>):(
                    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                      {apps.map(({grant,id,notes})=>{
                        const dl=getDeadlineStatus(grant.close);
                        return (
                          <div key={id} style={{ background:"#fff", borderRadius:12, border:"1px solid #D1E5D8", padding:"16px 20px", display:"flex", gap:16, alignItems:"center" }}>
                            <div style={{ flex:1 }}>
                              <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:17, fontWeight:700, color:"#0F3D2A" }}>{grant.name}</div>
                              <div style={{ fontSize:12, color:"#888", marginBottom:notes?6:0 }}>{grant.org} \u00b7 {grant.amount}</div>
                              {notes&&<div style={{ fontSize:12, color:"#5D7A6B", background:"#EAF5EE", padding:"5px 10px", borderRadius:6, marginTop:4 }}>{notes}</div>}
                            </div>
                            <div style={{ display:"flex", flexDirection:"column", gap:8, alignItems:"flex-end", flexShrink:0 }}>
                              <span style={{ background:dl.color+"20", color:dl.color, fontSize:12, fontWeight:600, padding:"3px 10px", borderRadius:12 }}>{dl.label}</span>
                              <select value={status} onChange={e=>updateAppStatus(id,e.target.value)} style={{ padding:"5px 10px", borderRadius:7, border:"1px solid #C8DDD0", fontSize:12, fontFamily:"'DM Sans',sans-serif", background:"#fff", cursor:"pointer" }}>
                                {["Not Started","In Progress","Submitted"].map(s=><option key={s} value={s}>{s}</option>)}
                              </select>
                              <button onClick={()=>{setInput(`Help me write a grant proposal for ${grant.name} by ${grant.org}. Amount: ${grant.amount}. My project is...`);setActiveTab("assistant");}} style={{ padding:"5px 12px", borderRadius:7, border:"none", background:"#2F7A52", color:"#0F3D2A", fontSize:12, cursor:"pointer", fontWeight:600, fontFamily:"'DM Sans',sans-serif" }}>AI Draft {"\u2192"}</button>
                            </div>
                          </div>
                        );
                      })}
        ...
        {activeTab==="assistant" && (
          <div style={{ maxWidth:760, margin:"0 auto" }}>
            <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:32, fontWeight:700, marginBottom:6, color:"#0F3D2A" }}>AI Grant Assistant</h1>
            <p style={{ color:"#5D7A6B", fontSize:14, margin:"0 0 20px" }}>Ask about eligibility, get grant recommendations, draft proposals and artist statements</p>
            <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:20 }}>
              {["Which grants am I eligible for as a South Asian diaspora filmmaker in Toronto?","Draft an artist statement for Soso Park for the MAC Matchmaker grant","What are the most urgent upcoming deadlines?","Help me write a project summary for Son of Soil"].map(p=>(
                <button key={p} onClick={()=>setInput(p)} style={{ padding:"8px 14px", borderRadius:20, border:"1.5px solid #2F7A52", background:"#EAF5EE", color:"#1E4A3A", fontSize:13, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", fontWeight:500 }}>
                  {p.length>50?p.slice(0,50)+"\u2026":p}
                </button>
              ))}
            </div>
            <div style={{ background:"#fff", borderRadius:16, border:"1px solid #D1E5D8", boxShadow:"0 2px 20px rgba(0,0,0,0.06)", overflow:"hidden" }}>
              <div style={{ height:460, overflowY:"auto", padding:"24px 24px 12px" }}>
                {messages.map((m,i)=>(
                  <div key={i} style={{ marginBottom:18, display:"flex", justifyContent:m.role==="user"?"flex-end":"flex-start" }}>
                    {m.role==="assistant"&&<div style={{ width:30, height:30, borderRadius:8, background:"#2F7A52", color:"#FFFFFF", fontSize:14, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", marginRight:10, flexShrink:0, fontFamily:"'Cormorant Garamond',serif" }}>C</div>}
                    <div style={{ background:m.role==="user"?"#2F7A52":"#EAF5EE", color:m.role==="user"?"#FFFFFF":"#1A2D24", padding:"12px 16px", borderRadius:m.role==="user"?"14px 14px 4px 14px":"14px 14px 14px 4px", maxWidth:"78%", fontSize:14, lineHeight:1.65, whiteSpace:"pre-wrap" }}>{m.content}</div>
                  </div>
                ))}
                {loading&&<div style={{ display:"flex", gap:10, alignItems:"center", padding:"8px 0" }}>
                  <div style={{ width:30, height:30, borderRadius:8, background:"#2F7A52", color:"#FFFFFF", fontSize:14, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Cormorant Garamond',serif" }}>C</div>
                  <div style={{ display:"flex", gap:5 }}>{[0,1,2].map(i=><div key={i} style={{ width:7, height:7, borderRadius:"50%", background:"#2F7A52", animation:"pulse 1.2s ease-in-out infinite", animationDelay:`${i*0.2}s` }}/>)}</div>
                </div>}
                <div ref={chatEndRef}/>
              </div>
              <div style={{ padding:"14px 18px", borderTop:"1px solid #E0EEE5", display:"flex", gap:10 }}>
                <textarea value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendMessage();}}} placeholder="Ask about grants, eligibility, or request a draft..." rows={2}
                  style={{ flex:1, padding:"10px 14px", borderRadius:10, border:"1.5px solid #C8DDD0", fontSize:14, fontFamily:"'DM Sans',sans-serif", resize:"none", outline:"none", background:"#FAFEFB", color:"#1A2D24", lineHeight:1.5 }}/>
                <button onClick={sendMessage} disabled={loading||!input.trim()} style={{ padding:"10px 20px", borderRadius:10, border:"none", background:loading||!input.trim()?"#C8DDD0":"#2F7A52", color:loading||!input.trim()?"#888":"#FFFFFF", fontWeight:600, fontSize:14, cursor:loading||!input.trim()?"not-allowed":"pointer", fontFamily:"'DM Sans',sans-serif", alignSelf:"flex-end" }}>Send</button>
              </div>
            </div>
        ...
        {activeTab==="contact" && (
          <div style={{ maxWidth:760, margin:"0 auto" }}>
            <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:32, fontWeight:700, marginBottom:8, color:"#0F3D2A" }}>Contact</h1>
            <p style={{ color:"#5D7A6B", fontSize:14, marginBottom:28 }}>Get in touch with the team behind CanGrants.</p>

            <div style={{ background:"#FFFFFF", border:"1px solid #D1E5D8", borderRadius:16, padding:"28px 32px", marginBottom:20, boxShadow:"0 2px 12px rgba(47,122,82,0.05)" }}>
              <div style={{ fontSize:11, letterSpacing:"2px", color:"#2F7A52", textTransform:"uppercase", marginBottom:6, fontWeight:700 }}>Company</div>
              <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:28, fontWeight:700, color:"#0F3D2A", marginBottom:6 }}>BetterHalf Labs</div>
              <p style={{ color:"#3F5A48", fontSize:14, lineHeight:1.7, margin:"0 0 10px" }}>
                A Toronto-based studio building practical tools for artists and producers. CanGrants is our first product — a single search platform for Canadian and international arts funding.
              </p>
              <div style={{ display:"flex", gap:16, flexWrap:"wrap", fontSize:14, marginTop:10 }}>
                <a href="mailto:hello@betterhalflabs.com" style={{ color:"#2F7A52", fontWeight:600, textDecoration:"none" }}>hello@betterhalflabs.com</a>
                <span style={{ color:"#5D7A6B" }}>\u00b7 Toronto, Canada</span>
                <a href="https://betterhalflabs.com" target="_blank" rel="noopener noreferrer" style={{ color:"#2F7A52", fontWeight:600, textDecoration:"none" }}>betterhalflabs.com {"\u2197"}</a>
              </div>
            </div>

            <div style={{ background:"#FFFFFF", border:"1px solid #D1E5D8", borderRadius:16, padding:"28px 32px", marginBottom:20, boxShadow:"0 2px 12px rgba(47,122,82,0.05)" }}>
              <div style={{ fontSize:11, letterSpacing:"2px", color:"#2F7A52", textTransform:"uppercase", marginBottom:14, fontWeight:700 }}>Social Media</div>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:12 }}>
                {[
                  { name:"Instagram", handle:"@betterhalflabs", url:"https://www.instagram.com/betterhalflabs/" },
                  { name:"LinkedIn", handle:"canadianartgrants", url:"https://www.linkedin.com/company/canadianartgrants" },
                  { name:"X / Twitter", handle:"@CdnArtGrants", url:"https://x.com/CdnArtGrants" },
                ].map(s => (
                  <a key={s.name} href={s.url} target="_blank" rel="noopener noreferrer" style={{ display:"block", padding:"14px 16px", borderRadius:10, border:"1px solid #D1E5D8", background:"#FAFEFB", textDecoration:"none", transition:"all 0.15s ease" }}>
                    <div style={{ fontSize:12, color:"#2F7A52", fontWeight:700, letterSpacing:"0.5px", textTransform:"uppercase", marginBottom:4 }}>{s.name}</div>
                    <div style={{ fontSize:14, color:"#0F3D2A", fontWeight:600 }}>{s.handle} {"\u2197"}</div>
                  </a>
                ))}
              </div>
            </div>

            <div style={{ background:"#EAF5EE", border:"1px solid #C8DDD0", borderRadius:16, padding:"22px 26px", marginBottom:20 }}>
              <div style={{ fontSize:11, letterSpacing:"2px", color:"#2F7A52", textTransform:"uppercase", marginBottom:8, fontWeight:700 }}>Security Notice</div>
              <p style={{ color:"#1E4A3A", fontSize:13, lineHeight:1.7, margin:0 }}>
                CanGrants is currently in early access. Accounts and saved data are stored locally in your browser only — no server-side database is in use yet. Please do <strong>not</strong> reuse a password from another service. A full authentication system (Supabase Auth / Clerk) is on our short-term roadmap.
              </p>
            </div>

            <div style={{ background:"#FFFFFF", border:"1px solid #D1E5D8", borderRadius:16, padding:"22px 26px", boxShadow:"0 2px 12px rgba(47,122,82,0.05)" }}>
              <div style={{ fontSize:11, letterSpacing:"2px", color:"#2F7A52", textTransform:"uppercase", marginBottom:8, fontWeight:700 }}>Press &amp; Partnerships</div>
              <p style={{ color:"#3F5A48", fontSize:14, lineHeight:1.7, margin:"0 0 6px" }}>
                Are you a funder, arts council, or media outlet? We'd love to hear from you. Email <a href="mailto:hello@betterhalflabs.com" style={{ color:"#2F7A52", fontWeight:600, textDecoration:"none" }}>hello@betterhalflabs.com</a> and we'll get back to you within two business days.
              </p>
            </div>
          </div>
        )}
      ...
      <footer style={{ borderTop:"1px solid #D1E5D8", padding:"24px 40px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:16, marginTop:40, background:"#fff" }}>
        <CanGrantsLogoImg size="md" />
        <div style={{ textAlign:"right", fontSize:12, color:"#888" }}>
          <div style={{ color:"#2F7A52", fontWeight:600, fontSize:14, fontFamily:"'Cormorant Garamond',serif" }}>CanGrants</div>
          <div>{"\u00A9"} 2026 BetterHalf Labs {"\u00b7"} Toronto, Canada {"\u00b7"} betterhalflabs.com</div>
      ...
      {selectedGrant&&(
        <div onClick={()=>setSelectedGrant(null)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:"#fff", borderRadius:18, maxWidth:600, width:"100%", maxHeight:"85vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.3)" }}>
            <div style={{ background:selectedGrant.location==="Canada"?"#0F3D2A":"#1F5277", padding:"22px 26px", borderRadius:"18px 18px 0 0" }}>
              <div style={{ fontSize:11, letterSpacing:"1.5px", color:selectedGrant.location==="Canada"?"#A8DCC0":"#9CC3DF", textTransform:"uppercase", marginBottom:6 }}>{selectedGrant.location} \u00b7 {selectedGrant.discipline.join(", ")}</div>
              <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:24, fontWeight:700, color:"#FFFFFF" }}>{selectedGrant.name}</div>
              <div style={{ fontSize:14, color:"#D1E5D8", marginTop:4 }}>{selectedGrant.org}</div>
            </div>
            <div style={{ padding:"24px 26px" }}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:20 }}>
                {[{label:"Amount",value:selectedGrant.amount},{label:"Deadline",value:selectedGrant.close==="Rolling"?"Rolling":new Date(selectedGrant.close).toLocaleDateString("en-CA",{month:"long",day:"numeric",year:"numeric"})},{label:"Opens",value:new Date(selectedGrant.open).toLocaleDateString("en-CA",{month:"long",day:"numeric",year:"numeric"})},{label:"Location",value:selectedGrant.location}].map(({label,value})=>(
                  <div key={label} style={{ background:"#EAF5EE", borderRadius:10, padding:"12px 16px" }}>
                    <div style={{ fontSize:11, color:"#888", marginBottom:2 }}>{label}</div>
                    <div style={{ fontSize:15, fontWeight:600, color:"#0F3D2A" }}>{value}</div>
                  </div>
                ))}
              </div>
              <div style={{ marginBottom:18 }}>
                <div style={{ fontSize:12, fontWeight:600, color:"#2F7A52", letterSpacing:"1px", textTransform:"uppercase", marginBottom:8 }}>Description</div>
                <p style={{ margin:0, fontSize:14, color:"#2A3A30", lineHeight:1.7 }}>{selectedGrant.description}</p>
              </div>
              <div style={{ marginBottom:18 }}>
                <div style={{ fontSize:12, fontWeight:600, color:"#2F7A52", letterSpacing:"1px", textTransform:"uppercase", marginBottom:8 }}>Eligibility</div>
                <p style={{ margin:0, fontSize:14, color:"#2A3A30", lineHeight:1.7 }}>{selectedGrant.eligibility}</p>
              </div>
              <div style={{ marginBottom:20 }}>
                <div style={{ fontSize:12, fontWeight:600, color:"#2F7A52", letterSpacing:"1px", textTransform:"uppercase", marginBottom:8 }}>Tags</div>
                <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                  {selectedGrant.tags.map(t=><span key={t} style={{ background:"#E3F2EA", color:"#2A5C2A", fontSize:12, padding:"4px 10px", borderRadius:20, fontWeight:500 }}>{t}</span>)}
                </div>
              </div>
              <div style={{ display:"flex", gap:10, marginTop:20 }}>
                <a href={selectedGrant.url} target="_blank" rel="noopener noreferrer" style={{ flex:1, padding:"12px 0", borderRadius:10, border:"none", background:"#2F7A52", color:"#0F3D2A", fontSize:14, fontWeight:700, textDecoration:"none", textAlign:"center", cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>Apply Now {"\u2197"}</a>
                <button onClick={()=>setSelectedGrant(null)} style={{ flex:1, padding:"12px 0", borderRadius:10, border:"1.5px solid #C8DDD0", background:"transparent", fontSize:14, cursor:"pointer", color:"#3F5A48", fontFamily:"'DM Sans',sans-serif", fontWeight:500 }}>Close</button>
              </div>
            </div>
          </div>
        </div>
